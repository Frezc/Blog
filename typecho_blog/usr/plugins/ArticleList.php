...
class ArticleList implements Typecho_Plugin_Interface
{
     ...
     * @param string $format
     */
    public static function hot($format='<li><a href="{permalink}"><span style="color:#f9962b">{commentsNum}℃</span>&nbsp;{title}</a></li>'){
    ...
    public static function random($format='<li><a href="{permalink}">{title}</a></li>'){
        ...
        if($xml1 && $option->rndtime!=0 && time()-$xml1->attributes()<$option->rndtime){
            ...
            if($option->mode == 'all'){
                ...
                foreach($rand as $index){
                    $result = $db->fetchRow($db->select('table.contents.cid','table.contents.title','table.contents.slug','table.contents.type')->from('table.contents')
                    ->join('table.relationships', 'table.relationships.cid = table.contents.cid',Typecho_Db::INNER_JOIN)
                    ->where('table.contents.status = ?', 'publish')
                    ->where($sql)
                    ->offset($index)
