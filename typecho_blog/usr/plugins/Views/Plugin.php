 ...
 * @version 1.0.1
 * @update: 2012.10.29
 * @link http://www.cgrabbit.info/blog/
 */
class Views_Plugin implements Typecho_Plugin_Interface
{
     ...
     * @return string
     */
    public static function theViews($before = 'Views: ', $after = ' ', $echo = 1)
    {
        $db = Typecho_Db::get();
        $cid = Typecho_Widget::widget('Widget_Archive')->cid;
        $row = $db->fetchRow($db->select('views')->from('table.contents')->where('cid = ?', $cid));
     ...
     * @return string
     */
    public static function theMostViewed($limit = 10, $before = '<br/> - ( Views: ', $after = '  ) ')
    {
        $db = Typecho_Db::get();
